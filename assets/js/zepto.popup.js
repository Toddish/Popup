/*-------------------------------
	...
	Simple Popup plugin for Zepto

	@author Todd Francis; King
	@version 2.2.3
...
;(function($, window){
	...
	$.Popup = function(settings)
	{
		var p = this,
			defaults = {
				...
				show : function($popup, $back){
					...
					$popup
						.anim({opacity : 1}, plugin.o.duration, "linear", function(){
				...
				hide : function($popup, $back){
					...
					if( $popup !== undefined ){
						...
						// Fade the popup out
						$popup.anim({opacity : 0}, this.o.duration);
				...
				types : {
					...
					Zepto					: function(content, callback){
		...
		p.open = function(content, popupType, ele){
			...
			// If it's not inline, Zepto or a function
			// it might have params, and they are top priority
			if( $.inArray(popupType, ['inline', 'Zepto', 'function']) === -1 ){
		...
		p.close = function(){
			...
			if( $back !== undefined ){
				...
				// Fade out the back
				$back.anim({opacity : 0}, p.o.duration, "linear", function(){
		...
		p.getCenter = function(){
			var pW = $pCont.children().width(),
				pH = $pCont.children().height(),
